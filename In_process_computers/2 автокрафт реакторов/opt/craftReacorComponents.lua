...
function transfer_fluid(count, name_trans, fluid_cost)
    ...
    local result = 0
    constants.gpu.set(34, 16, "Стоимость в материи " .. mb_cost .. " mb")
    while result < mb_cost do
        local _, c = name_trans.transferFluid(constants.sides.down, constants.sides.up, mb_cost - result)
        result = result + c
        constants.gpu.set(34, 18, "Перенесено " .. result .. " mb. Не хватает " .. mb_cost - result .. "mb.")
        os.sleep(5)
    end
end
...
function check_or_wait(state)
    for component, craft in pairs(constants.trans_craft) do
        constants.gpu.fill(34, 12, 50, 20, " ")
        constants.gpu.set(34, 12, "Компонент: " .. component)
        constants.gpu.set(34, 14, "Сканирование сундука...")
...
function wait_need_items(craft, component, item_count)
    while craft.count > item_count do
        ...
        constants.gpu.set(34, 14, "Осталось: " .. count_craft .. " штук.")
        os.sleep(3)
    end
end
...
function check_need_items(craft, component, item_count)
    if craft.count > item_count then
        ...
        constants.gpu.set(34, 14, "Осталось создать: " .. count_craft .. " штук")

        transfer_fluid(craft.count - item_count, craft.transposer, craft.mb)
    else
        constants.gpu.set(34, 14, "Крафт: " .. component .. " не требуется")
    end
end

function check_redstone()
    local count_fluid = constants.main_trans_craft.getTankLevel(constants.sides.up, 1)
...
function craftReactor.main()
    ...
    check_or_wait("check")
    constants.gpu.fill(34, 12, 50, 20, " ")
    constants.gpu.set(34, 12, "Ожидание завершения крафта компонентов....")
    check_or_wait("wait")
    constants.gpu.fill(34, 12, 50, 20, " ")
    constants.gpu.set(34, 12, "Все компоненты готовы. Проверка количества жидкого красного камня...")
    check_redstone()
    constants.gpu.fill(34, 12, 50, 20, " ")
    constants.gpu.set(34, 12, "Все приготовления завершены. Запуск робота для постройки реактора.")
    --перед настройкой робота надо еще сделать статус работы (создание компонентов, ожидание создания, ожидание редстоуна)
